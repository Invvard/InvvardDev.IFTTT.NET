    ...
    where T : class, new()
{
    ...
    public static string Serialize(T data) => Serialize(data, JsonSerializerOptions);

    public static string Serialize(T data, JsonSerializerOptions options)
    {
        //options.Converters.Add(new TopLeveBaseDataConverter<T>());
        return JsonSerializer.Serialize(new TopLevelMessageModel<T>(data), options);
    }

    public static TopLevelMessageModel<T> Deserialize(string json) => Deserialize(json, JsonSerializerOptions);
...
{
    public static string Serialize(IList<ErrorMessage> errors) => Serialize(errors, JsonSerializerOptions);

    public static string Serialize(IList<ErrorMessage> errors, JsonSerializerOptions options) => JsonSerializer.Serialize(new TopLevelErrorModel(errors), options);
}
